...
inline
unsigned int lookupHash(unsigned int fileStrcode, unsigned int folderStrcode) {
	unsigned int newHash = fileStrcode;
	if (!folderStrcode) return fileStrcode;

	folderStrcode = (folderStrcode >> 4) ^ (folderStrcode << 4);
	folderStrcode ^= 0x10EA;
	folderStrcode = ~folderStrcode;

	newHash = folderStrcode ^ fileStrcode;
